...
describe("E2E simulation test", () => {
  ...
  describe("when terrain and cultures are being created", () => {
    beforeEach(() => {
      ...
      cells = generateIds(3).map((id) => CellFactory.createCell(id, terrain));
  ...
  describe("when resources are available for the cell", () => {
    beforeEach(() => {
      ...
      cells = [CellFactory.createCell("c0", terrain)];
      simulation = new Simulation(terrain, cells);
      simulation.step();
    });
  ...
  describe("when there is no free spot for the cell to migrate", () => {
    beforeEach(() => {
      ...
      cells = generateIds(4).map((id) => CellFactory.createCell(id, terrain));
      simulation = new Simulation(terrain, cells);
  ...
  describe("when cell has no energy", () => {
    beforeEach(() => {
      ...
      cells = [CellFactory.createCell("c0", terrain, { initialEnergy: 0.2 })]; // cell will consume 0.2 for inner processes, so no energy for migration
      simulation = new Simulation(terrain, cells);
  ...
  describe("when cell has too much energy", () => {
    beforeEach(() => {
      terrain = TerrainFactory.createTerrain({ width: 1, height: 1, resourcesPerSpot: 1 });
      cells = [CellFactory.createCell("c0", terrain, { initialEnergy: 50 })];
      simulation = new Simulation(terrain, cells);
  ...
  describe("when cell has enough energy to divide", () => {
    beforeEach(() => {
      ...
      cells = [CellFactory.createCell("c0", terrain, { initialEnergy: 1 })];
      simulation = new Simulation(terrain, cells);
  ...
  describe("when cell has not enough energy to divide", () => {
    beforeEach(() => {
      terrain = TerrainFactory.createTerrain({ width: 1, height: 2, resourcesPerSpot: 0 });
      cells = [CellFactory.createCell("c0", terrain, { initialEnergy: 1, initialSize: 8 })];
      simulation = new Simulation(terrain, cells);
